...
from capitulo_13_proyecto_1_alienigenas.ejercicios.stars import Star
...
class AlienInvasion:
    ...
    def _create_universe(self):
        """Crea un universo de estrellas."""
        # Hace una estrella y calculamos el número de estrellas que hacen caben en una fila.
        # El espacio entre estrellas es igual al ancho de una estrella.
        star = Star(self)
        star_width, star_height = star.rect.size
        available_space_x = self.settings.screen_width - (2 * star_width)
        number_stars_x = available_space_x // (2 * star_width)
        # Determina el número de filas de estrellas que caben en la pantalla.
        ship_height = self.ship.rect.height
        available_space_y = (self.settings.screen_height -
                             (3 * star_height) - ship_height)
        number_rows = available_space_y // (2 * star_height)

        # Crea el universo completo de estrellas
        for row_number in range(number_rows):
            for star_number in range(number_stars_x):
                star_number = randint(-10, 10)
                row_number = randint(-10, 10)
                self._create_star(star_number, row_number)

    def _create_star(self, star_number, row_number):
        """Crea una estrella y la posiciona en la fila."""
        star = Star(self)
        star_width, star_height = star.rect.size
